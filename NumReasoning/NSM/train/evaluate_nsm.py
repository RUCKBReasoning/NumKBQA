    ...
    def evaluate(self, valid_data, test_batch_size=20, write_info=False):
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            with torch.no_grad():
                ...
                if self.use_num:
                    num_batch = valid_data.get_num_batch()
                    loss, extras, pred_dist, tp_list = self.student.forward_num(num_batch)
            ...
            question_types = valid_data.get_batch_question_types()
            question_type_list.extend(question_types)
        ...
        print(case_ct)
        res_str, superlative_acc = calc_type_accuracy(question_type_list=question_type_list, hits_list=hits)
        print(res_str)
        if write_info:
            self.file_write.close()
            self.file_write = None
        return np.mean(f1s), np.mean(hits), res_str, superlative_acc
